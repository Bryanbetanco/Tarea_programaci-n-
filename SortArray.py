...
def median(self):
    if self.__nItems == 0:  # Verificar si el array está vacío
        return None

    # Crear una copia ordenada de los elementos actuales
    sorted_array = sorted(self.__a[:self.__nItems])  # Ordenar solo los elementos válidos
    mid = self.__nItems // 2  # Calcular la posición media

    # Retornar el valor de la mediana según si el número de elementos es impar o par
    if self.__nItems % 2 == 1:  # Si es impar, devolver el elemento medio
        return sorted_array[mid]
    else:  # Si es par, devolver el promedio de los dos elementos centrales
